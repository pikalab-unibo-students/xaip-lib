...
class ExplanationTest : AnnotationSpec() {
    ...
    fun testQuestion3() {
        ...
        fun createNewFluent(action: Operator, predicate: Predicate): Fluent = // new predicate
            Fluent.positive(predicate, *action.args.toTypedArray()) // TODO(fixa sta roba con gli argomenti delle azioni)
        ...
        val HProblem = Problem.of( // problem extended
            ...
            goal = FluentBasedGoal.of(
                mutableSetOf(newFluent).also { it.addAll((questionAddActionPlan.problem.goal as FluentBasedGoal).targets) }
            ) // extended
        )
